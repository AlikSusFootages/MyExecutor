...
function Dozer:Start()
    if game:isLoaded() then
                ...
                for _, button in ScriptCard:GetChildren() do
                    if button:IsA("CanvasGroup") then
                        ScriptCard[button.Name].ImageButton.MouseButton1Click:Connect(function()
                            ...
                            if button.Name == "RunIcon" then
                                runcode(script)
                            elseif button.Name == "TrashIcon" then
                                DelScript(name)
                                UpdateScripts = GetScripts()
                            end
                        end)
                    end
                end
        ...
        SavedScriptss.CanvasGroup.ImageButton.MouseButton1Click:Connect(function()
            CreateRipple(Mouse.X, Mouse.Y, SavedScriptss.CanvasGroup, UsingTheme.Main)
        end)
